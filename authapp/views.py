...
# View for Referral History
# Masking of the referee
class ReferralHistory(generics.ListAPIView):
    ...
    def get(self,request,format=None):
        serializer = ReferralSerializer(Referrals.objects.filter(referred_by = request.user),many=True)
        if(serializer.is_valid):
            print(serializer.data)
            newdict=serializer.data
            for x in range(len(serializer.data)):
                user2 = serializer.data[x]['referee']
                masked = user2
                lo = masked.find('@')
                if lo>0:
                    masked = masked[0] + '*******' + masked[lo-1] + '@' + masked[lo+1] + "*****" + masked[-1]
                newdict[x]['referee'] = masked
        try:
            return Response(newdict, status=status.HTTP_200_OK)
        except:
            return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)       

# View getting the referral code of the logged in user
